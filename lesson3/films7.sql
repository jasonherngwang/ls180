...
-- Question 2: Convert 1:M to M:M
/*
- Create join table directors_films, columns film_id and director_id.
- Change `films` FK constraint to reference the join table.
- Add FK to `directors`, referencing the join table.
*/
...
-- Question 3: Fill out join table
...
SELECT f.title,
       d.name
  FROM directors AS d
  JOIN directors_films AS d_xref_f
    ON d.id = d_xref_f.director_id
  JOIN films AS f
    ON f.id = d_xref_f.film_id
 ORDER BY f.title;
...
SELECT d.name AS director,
       count(d_xref_f.film_id) AS films
  FROM directors AS d
  JOIN directors_films AS d_xref_f
    ON d.id = d_xref_f.director_id
 GROUP BY d.name
